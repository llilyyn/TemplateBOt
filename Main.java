...
public class Main {
 ...
 static String [] farewell = {
     "In that case, it seems like it's time for us to say goodbye. Adieu!", "Well, that sure was fun! Farewell!", "Okay then. It was fun chatting with you. Bye!", "I see. Then I'll talk to you later!", "Okay, farewell!", "Got it. In that case, I bid you adieu!", "Alright then, catch you later!", "Understood. Then I'll catch you on the flip side!"
 };
 ...
 static Meals[] mealList = {
     ...
     new Meals("bowl", 9.25, 900, 18, 110, 28),
     ...
     while (!byeBye && !containsGoodbye(userResp)) {
         System.out.println(checkIt(userResp));
          if (opt == 1) { // Check if the user is asking about toppings
             System.out.println("Would you like to know the nutritional info about any of the toppings? If so, which one?");
             next = 2;
             ...
             System.out.println("Would you like to know the nutritional info about any of the drinks? If so, which one?");
             next = 2;
             userResp = in.nextLine();
             if (next == 2) {
             ...
             System.out.println("Would you like to know the nutritional info about any of the meals? If so, which one?");
             next = 2;
             userResp = in.nextLine();
             if (next == 2) {
             ...
             System.out.println("Would you like to know the nutritional info about any of the dips? If so, which one?");
             next = 2;
             userResp = in.nextLine();
             if (next == 2) {
 ...
 public static String checkIt(String statement) {
     ...
     if (statement.contains("meal") || statement.contains("combo") || statement.contains("dish") || statement.contains("food item") || statement.contains("meals")) {
         response = containing(statement, "meals") + " We have several options like burritos, quesadillas, and tacos.";
         opt = 3;
     }
     ...
     if (statement.contains("protein") || statement.contains("meat") || statement.contains("meats")) {
         response = containing(statement, "protein") + " Our protein options include chicken, steak, carnitas, barbacoa, and sofritas.";
         opt = 5;
     }
      return response;
 }
 ...
 public static String getMealPrice(String statement) {
     for (Meals meal : mealList) {
         if (statement.toLowerCase().contains(meal.getName().toLowerCase())) {
             return "A " + meal.getPriceInfo() + " What else would you like to know?";
     }
    }
